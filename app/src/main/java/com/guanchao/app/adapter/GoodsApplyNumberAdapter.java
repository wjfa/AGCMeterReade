...
public class GoodsApplyNumberAdapter extends MyBaseAdapter<String> {

    private int clickTemp = -1;//刚进入时临时的点击背景
    public GoodsApplyNumberAdapter(Context context,int clickTemp) {
        super(context);
    ...
    public View getMyView(final int position, View convertView, ViewGroup parent) {
        ...
        //设置物品到控件上
        String goodsApply = myList.get(position);
        holder.goods.setText( goodsApply);
    ...
    //声明监听接口对象
    public GoodsApplyNumberAdapter.onNewItemClickListener myListener;

    //对外提供一个监听方法
    public void setonNewItemClickListener(GoodsApplyNumberAdapter.onNewItemClickListener listener) {
